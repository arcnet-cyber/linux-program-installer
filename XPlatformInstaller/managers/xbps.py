...
class XbpsManager(PackageManager):
    def search_package(self, name):
        result = subprocess.run(
            ["xbps-query", "-Rs", name], capture_output=True, text=True
        )
        return result.stdout.strip()

    def validate_package(self, name):
        # Try to find exact match using regex anchors
        result = subprocess.run(
            ["xbps-query", "-Rs", f"^{name}$"], capture_output=True, text=True
        )
    ...
    def generate_install_command(self, package_list):
        return f"sudo xbps-install -y {' '.join(package_list)}"
